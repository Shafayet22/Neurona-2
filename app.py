...
def handle_investment_request(request_id, action):
    ...
    def _handle_request(conn, request_id, user_id, action):
        ...
        # If approved, add funding to creator's wallet
        if action == 'approve':
            conn.execute('''
                INSERT INTO wallet_transactions (user_id, type, amount, description)
                VALUES (?, ?, ?, ?)
            ''', (user_id, 'funding_received', investment_request['investment_amount'],
                  f'Investment received for "{investment_request["title"]}"'))
            ...
            conn.execute('''
                ...
                SET balance = balance + ?
                WHERE user_id = ?
            ''', (investment_request['investment_amount'], user_id))
    ...
    def _get_investor_dashboard_data(conn, user_id):
        ...
        portfolio_stats = conn.execute("""
            SELECT 
                ...
                AVG(CASE WHEN amount > 0 THEN 1.0 ELSE 0.0 END) * 100 as success_rate
            FROM investments 
            WHERE investor_id = ?
        """, (user_id,)).fetchone()
        ...
        return ideas, categories, stages, pending_count, {
